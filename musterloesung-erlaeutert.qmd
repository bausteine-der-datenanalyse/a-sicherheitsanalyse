...
d_zaehle_unfaelle <- d_unfaelle_bab_2023 |>
  ...
  group_by(UKATEGORIE) |>
  summarise(
    A38 = sum(nearest_autobahn_name == "A38"),
    Autobahnen = n(),
    .groups = "drop"
  ) |>
...
d_unfaelle_bab_alle <- d_unfaelle_alle |>
  ...
  filter(as.double(nearest_autobahn_distance) <= 20) |>
  select(UKATEGORIE, nearest_autobahn_name, abschnitt_id, jahr)
```
