---
title: "Verkehrsunfälle in Bochum"
format: 
  html:
    page-layout: custom
server: shiny
...
# CSV-Datei einlesen und aufbereiten
...
# Bochum als Begrenzung
d_bochum <- gisco_get_nuts(nuts_id = "DEA51", nuts_level = 3, resolution = 03) |>
  st_transform(25832)

data_sf <- st_as_sf(data, coords = c("LINREFX", "LINREFY"), crs = 25832)
...
checkboxGroupInput(
  "kategorie",
  label = h3("Unfallkategorien"),
  choices = list(
    "Unfälle mit Getöteten" = 1,
    "Unfälle mit Schwerverletzten" = 2,
    "Unfälle mit Leichtverletzten" = 3
  ),
  selected = 1:3
)
...
selectInput(
  "beteiligung",
  label = h3("Verkehrsbeteiligung"),
  choices = list(
    "Alle Unfälle" = 1,
    "Unfälle mit PKW-Beteiligung" = 2,
    "Unfälle mit Radfahrer-Beteiligung" = 3,
    "Unfälle mit Fußgänger-Beteiligung" = 4
  ),
  selected = 1
)
...
# Filter reagiert auf Nutzereingabe
Bochum_Filter <- reactive({
  testfilter <- Bochum_Unfall |>
    filter(
...
output$mymap <- renderLeaflet({
  ...
  ## Farbeinstellungen
  pal <- colorFactor(
    c("#04B431", "yellow", "red", "white", "#00BFFF", "orange", "black"),
    domain = 1:7
  )
  ...
  leaflet(Bochum_Filter()) |>
    ...
    addLegend(
      "bottomright",
      colors = c("#04B431", "yellow", "red", "white", "#00BFFF", "orange", "black"),
...
output$table <- renderTable(
  {
    Bochum_tbl <- Bochum_Filter() |>
      ...
      summarise(
        PKWUnfälle = sum(IstPKW == "1"),
        Radfahrerunfälle = sum(IstRad == "1"),
        Fußgängerunfälle = sum(IstFuss == "1")
      ) |>
...
output$downloadData <- downloadHandler(
  ...
  content = function(file) {
    write_csv2(Bochum_Filter(), file, row.names = FALSE)
  }
)
```
